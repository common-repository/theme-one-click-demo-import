 ...
 * @subpackage Importer
 */
if ( ! class_exists( 'TOC_XML_Importer' ) ) {
	class TOC_XML_Importer extends WP_Importer
	{
		...
		function __construct( $args ) {
			...
			$xml_upload_file = THEME_ONE_CLICK_DEMO_CONTENT_PATH.$args['file'];


			if ( file_exists( $xml_upload_file ) ) {

				// for windows systems
				$file = str_replace( '\\', '/', $xml_upload_file );
				$this->xmlfile = $file;
			}
		...
		function do_import() {
			...
			$file = get_attached_file( $this->xmlfile );
	...
	function import_start( $file ) {
		echo '<h3>' . __( 'XML Import Result', 'wordpress-importer' ) . '</h3>';
		if ( ! is_file($file) ) {
	...
	function import_end() {
		...
		echo '<div>' . __( 'Data Successfully Transfered', 'wordpress-importer' ) . '</div>';
	...
	function fetch_remote_file( $url, $post ) {
        ...
        $upload = wp_upload_bits($file_name, 0, wp_remote_retrieve_body($remote_response), $post['upload_date']);
		if ( $upload['error'] )
...
function TOC_xml_import( $args ) {

	$defaults = array( 'file' => '', 'map_user_id' => 0);
	$args = wp_parse_args( $args, $defaults );
	$autoimport = new TOC_XML_Importer( $args );

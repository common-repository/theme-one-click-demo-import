...
class TOC_WIE_Importer
{
	 ...
	 * @since 0.3
	 */
	function TOC_wie_upload_import_file($args) {

			$uploaded_file_name = $args['wie_file'];
			// Get the upload data.
			
			$uploaded_file = THEME_ONE_CLICK_DEMO_CONTENT_PATH.$uploaded_file_name;
			$file = str_replace( '\\', '/', $uploaded_file );
			// Process import file.
			$this->TOC_wie_process_import_file( $file);
	}
	 ...
	 */
	function TOC_wie_process_import_file( $file ) {
		//print_r($file);
		...
		$wie_import_results = $this->TOC_wie_import_data( $data );
	}
	 ...
	 */
	function TOC_wie_import_data( $data ) {
		...
		$available_widgets = $this->TOC_wie_available_widgets();
	 ...
	 */
	function TOC_wie_available_widgets() {
...
function TOC_wie_import( $args ) {

	$wieimport = new TOC_WIE_Importer();
	$wieimport->TOC_wie_upload_import_file($args);
	if ( TOC_wie_have_import_results() ) {
		
		TOC_wie_show_import_results();
	}
